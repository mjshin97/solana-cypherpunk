...
const PAYMENT_AMOUNT_SOL = 0.01; // Pay 버튼 클릭 시 전송할 SOL
...
const App: React.FC = () => {
  ...
  // Pay 버튼 클릭 시, 기존 연결된 Wallet(publicKey)을 사용해 전송
  const handlePay = async () => {
    ...
    try {
      ...
      // 기존 publicKey를 받는 쪽으로 전송 (자신 또는 다른 계정)
      // 여기서는 예시로 NFT 민팅한 사람(publicKey)에게 전송하도록 함
      // 필요 시 다른 계정을 지정할 수 있음
      const receiver = publicKey; // 기존 연결된 Wallet 활용
      ...
      const paymentSig = await connection.sendRawTransaction(
        signedPaymentTx.serialize()
      );
      await connection.confirmTransaction(paymentSig, "confirmed");

      console.log("Payment signature:", paymentSig);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-200 flex flex-col items-center justify-center px-4 py-10">
      ...
      <footer className="text-gray-500 mt-8 text-sm text-center">
